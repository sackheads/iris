...
struct JobFireIntegrationTests {
    ...
    func jobToolsCarryTheWatcherCallback() async throws {
        ...
        let background = try #require(state.conversations.first { $0.isBackground })
        #expect(background.title.hasPrefix("notes · "))
        let run = try #require(try store.ledger.runs(jobId: job.id, limit: 1).first)
        #expect(run.triggerKind == "fsEvent")
        #expect(run.status == .completed)
        #expect(run.transcriptConversationId == background.id)
        let prompt = background.history.first?.parts.compactMap(\.text).joined() ?? ""
        #expect(prompt.contains("/tmp/notes/a.txt"), "the paths that woke it reached the run")
        #expect(state.conversations.first { $0.id == conversationId }?.messages.isEmpty == true,
                "and the conversation the watch was created in heard nothing")
    }
