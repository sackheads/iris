...
actor IrisEngine {
    ...
    func fireHandler() -> JobScheduler.FireHandler {
        { [weak self] job, fire in
            guard let runner = await self?.jobRunner() else { return false }
            // `fire`, not `run`: the runner is the single admission point, so a due job meets the
            // same overlap, breaker and budget checks a watch fire does (§4). A replayed
            // occurrence is one of those ordinary cadence fires (R32), which is why the answer
            // goes back: anything short of a run ends the burst behind it.
            return await runner.fire(job: job, origin: .cadence(kind: fire.reason),
                                     note: fire.note) == .run
        }
    }
