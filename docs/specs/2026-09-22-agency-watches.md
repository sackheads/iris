...
2. **Changes during a run accumulate and re-fire once.** Watches default to overlap `queue`. Events that arrive while the watch has a fire outstanding are held; when the run ends, the runner's held re-fire carries the union of the paths it held and everything the coordinator gathered since, and fires once. Under `skip` the coordinator holds the same way and the next window after the run carries everything. *Why:* a change made during a run must not be silently lost, which is what happens today; and the job must never run concurrently with itself. *Cost if wrong:* a watch under `queue` runs twice for one edit session — bounded, and the second run carries only what the first did not see.
3. **Self-writes are any Iris file-tool write in this process.** Every tool that writes a filesystem path through `ToolExecutor` — today `write_file`, `create_skill`, `update_skill`, `delete_skill` — records the symlink-resolved path and the instant in one process-wide registry, from every conversation: foreground, job run, subagent, evaluator. An event on such a path within the expiry is dropped and counted, and so is the sibling temp file an atomic write creates. Commands (`run_command`, host or container) and MCP tools are not seen; the docs say so and the breaker stays the backstop. `--run-job` is another process holding the store lock, so no watch is live to filter for; its writes are not recorded and need not be. *Why:* the loop the design doc names (a watch summarising a folder into a file in that folder) is Iris reacting to itself whichever conversation wrote; the hook that knows a write's resolved path already exists. *Cost if wrong:* a user who wanted a watch to react to a foreground turn's write into the watched folder gets a count instead of a run.
...
| `pending: Set<String>` | paths accepted this burst |
| `heldPaths: Set<String>` | paths handed to a fire whose handler has not returned, plus anything accepted meanwhile |
| `fireOutstanding: Bool` | a handler call is in flight for this subscriber |
| `burstBegan: Date?`, `lastAccepted: Date?` | timers |
| `coalesced: Int`, `noise: Int`, `ownWrites: Int` | this burst (zero between bursts) |
| `absorbedSinceLaunch: AbsorbedCounts` | `{ noise: Int, ownWrites: Int }`, all bursts and between bursts, memory only |
...
**On a batch** (a stream root and the `[String]` its FSEvents callback delivered; one callback may deliver a path once with several flags OR'd, which counts once): the batch is fanned out once, to every subscriber whose watch root covers each path by lexical prefix after `standardizingPath` — event paths are never stat'ed or canonicalised individually, because the root is canonical and FSEvents reports paths under it, and a delete event names a path that no longer exists. For each subscriber and path: matches the built-in set or the subscriber's globs → `noise += 1`; else the registry says it is Iris's own (§4) → `ownWrites += 1`; else if `fireOutstanding` → insert into `heldPaths`; else insert into `pending`, set `burstBegan` if nil, `lastAccepted = now`, `coalesced += 1`. A batch that accepted nothing changes no timer.
...
1. Read the job row fresh from the ledger. Missing, `enabled == false`, or `pausedReason != nil` → drop `pending`, end the burst, count nothing (the job is not running by the user's choice; `sync` will remove the subscriber).
...
3. The handler returns the admission:
   ...
   - `.skipInFlight` (overlap `skip`) — the paths stay in `heldPaths`; the runner writes no row for a watch skip, as today. Nothing re-fires while the run lasts: the subscriber is not re-evaluated until the handler that started the run returns, then the held paths begin a new burst. This is what stops a busy directory calling admission every window for the length of a run.
...
At every instant a path is in exactly one of `pending`, `heldPaths`, or a started run.
