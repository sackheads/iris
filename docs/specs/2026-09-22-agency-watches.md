...
**On a batch** (a stream root and the `[String]` its FSEvents callback delivered; one callback may deliver a path once with several flags OR'd, which counts once): the batch is fanned out once, to every subscriber whose watch root covers each path by lexical prefix after `IrisPaths.canonicalPath` (R-D4-5) — the coverage test itself is lexical, never a `stat` of the event path, because the root is canonical and FSEvents reports paths under it, and a delete event names a path that no longer exists. For each subscriber and path: matches the built-in set or the subscriber's globs → `noise += 1`; else the registry says it is Iris's own (§4) → `ownWrites += 1`; else if `fireOutstanding` → insert into `heldPaths`; else insert into `pending`, set `burstBegan` if nil, `lastAccepted = now`, `coalesced += 1`. A batch that accepted nothing changes no timer.
