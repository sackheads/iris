...
4. **Gate substrate.** Recommendation: built-in gates only need no container; a script gate requires the `apple/container` runtime and is refused at creation when it is not installed. The runtime API grows a timeout and a mount list (it had one mount and no timeout), which is a prerequisite task, not a nice-to-have. **Accepted**, and landed — amendments 10 and 12. As built (R28), "installed" is the same question a `mutating` job is asked — the runtime installed *and* sandboxing switched on — and it is asked again at every evaluation, not only at creation: when the answer has since become no, the evaluation is a gate error and the script does not run on the host.
...
- **Prerequisite (landed — amendment 10):** `ContainerRuntime.createDetached` and `SandboxSessionManager.run` take `mounts: [String]` and `timeoutSeconds: Int?`; the host-side `run_command` timeout that the sandboxed branch used to drop is honoured there too (a bug fix in its own right).
