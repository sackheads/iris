...
11. R23 (2026-09-21): the §7 prerequisite has landed, so no command in this deliverable runs in the VM unbounded. `ContainerRuntime` takes `mounts: [String]` — one `--mount` per entry, rendered `type=virtiofs,source=…,target=…[,readonly]` from a `source[:target][:ro]` entry — and a per-command `timeoutSeconds` it enforces itself: at the deadline the CLI child's own children are killed first (they inherited the pipes and would otherwise hold the read open past it), then the child gets SIGTERM and, two seconds later, SIGKILL, and the call throws a timeout carrying the elapsed seconds. `run_command`'s already-clamped `timeout_seconds` is forwarded on the sandboxed branch and comes back in the host's words ("Error: command timed out after N seconds"); a deadline does not trigger the session's recreate-and-retry, which could only spend the same wall clock over again. A mount path containing a comma is refused rather than rendered, because `--mount`'s value is a comma-separated `key=value` list with no escape; a path containing a space needs nothing done to it, since each mount is one argv element and no shell is involved — cost if wrong: a directory whose name contains a comma has to be mounted by its parent.
