...
- "Approve and run" executes exactly the persisted call, once (`approvedAt` set atomically; the one-shot `approvedCalls` entry consumed on read), never resumes a model turn. The card renders the full call, and Vibecop's verdict on it is shown beside the button; a click overrides a DENY rather than skipping the evaluation.
