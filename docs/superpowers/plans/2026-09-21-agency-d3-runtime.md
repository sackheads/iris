...
- "Approve and run" executes exactly the persisted call, once (`approvedAt` set atomically), never resumes a model turn, never consults Vibecop.
- A gate "unchanged" writes a `completed` row with outcome "gate: no change" and no card.
