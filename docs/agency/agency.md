...
**1. Substrate: in the app.** Jobs and watches run inside the Iris process. The scheduler already survives sleep via `NSWorkspace.didWakeNotification` and recomputes `nextFireAt` after a fire, so a weekend asleep produces one catch-up run per job, not a stampede. We add a sleep assertion for the duration of a run, capped at the run's timeout, and a per-job `catchUp` policy: `coalesce` (default), `skip`, `replay`. `replay` is the stampede Clomp warned about with a different name; it carries a per-job cap on how many occurrences one catch-up replays (five) and is off unless a job asks for it. It replays the most recent missed occurrences rather than the oldest, one at a time through the ordinary admission path, and stops at the first refusal — so the breaker and the budgets bound a burst the same way they bound anything else.

**2. Unattended actions fail closed.** A background run never blocks on an approval. A tool call that would need one fails immediately with a structured status; the run turns that into a proposal card in the destination conversation ("job `pr-sweep` wants to run `git push origin main`; Approve and run / Dismiss"). Approving dispatches that one call, once, as a tracked run of its own — not a resumption of the job's turn — and no approval covers a call the job's own profile refused or a write into the directories where permission itself is granted. Every job declares a profile: `readOnly` runs autonomously; `mutating` runs in the `apple/container` sandbox unless the job carries an explicit waiver that I granted in the main conversation when the job was created. Deterministic jobs with no model involvement are the encouraged default.
...
- **Trigger.** `.schedule`, `.fsEvent`, `.poll`. A poll is a schedule with a gate — a URL, a path or a script — deciding at each occurrence whether there is anything to run; it exists as a trigger type so the model has a name for it. The old `ScheduledJob` and `WatcherRule` records are not imported into this model — nobody outside development had real ones, so deliverable 1 drops them (see below).
- **Gate.** A check that returns a signal and an optional payload. No model tokens are spent unless the gate says something changed. The gate is code the model wrote, and it runs on every tick with nobody watching, so it gets the same treatment as a mutating tool: either one of the built-in checks (HEAD of a URL — ETag, Last-Modified, Content-Length; or a path's mtime, size and hash, or the newest change under a directory) or a script run in the `apple/container` VM with the job's declared mounts, attached read-only. A script gate's verdict is the last line of its stdout — `CHANGED` or `UNCHANGED` — never its exit code, which `diff -q` and `grep -q` disagree about; a non-zero exit, a timeout or any other last line is a gate failure, and three in a row pause the job. A gate never executes free-form on the host: with the VM unavailable a script gate is a gate failure, not a host command. Creating one is Vibecop-reviewed like any mutating action. Gate output is untrusted input and passes the InjectionGuard tiers before any model sees it, the same path system events use today.
