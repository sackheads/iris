...
**1. Substrate: in the app.** Jobs and watches run inside the Iris process. The scheduler already survives sleep via `NSWorkspace.didWakeNotification` and recomputes `nextFireAt` after a fire, so a weekend asleep produces one catch-up run per job, not a stampede. We add a sleep assertion for the duration of a run, capped at the run's timeout, and a per-job `catchUp` policy: `coalesce` (default), `skip`, `replay`. `replay` is the stampede Clomp warned about with a different name; it carries a hard per-job cap on replays per wake (five) and is off unless a job asks for it. It replays the most recent missed occurrences rather than the oldest, one at a time through the ordinary admission path, and stops at the first refusal — so the breaker and the budgets bound a burst the same way they bound anything else.
