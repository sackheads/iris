...
*   **Session Control:** `/new` (fresh chat), `/clear` (clear current buffer), `/archive` (move this conversation to the archive), `/unarchive` (return it to the active list), `/stop` (cancel active goal/subagents), `/update` (check for GitHub releases).
