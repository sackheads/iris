...
When a model call fails, Iris shows a compact red system pill with a one-line headline (provider, HTTP status, and the provider's message). The provider's raw response is available behind a chevron, capped at 2 KB; the full body goes to the console log. Rate limits and overloads (HTTP 429, 503, 529) are retried automatically with a 2 s / 4 s / 8 s backoff before the error is shown, and each retry is announced as a `[retry]` system line.
