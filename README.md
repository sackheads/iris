...
*   **Per-Conversation Store:** Conversations live in `~/.iris/conversations.sqlite`, one row per message and per model-history entry, so appending a message costs one row write off the main thread instead of re-encoding every conversation you have ever had. An unreadable entry is quarantined and reported at launch rather than taking the whole list down with it. Existing conversations are imported once from the old settings blob on first launch; the blob is kept under a legacy key for a while and then retired.
