...
*   **Jobs:** `schedule_job` creates a job on a cron schedule (five fields, with a per-job IANA time zone) or a plain interval; `register_directory_watcher` creates a job that watches a directory via `FSEventStream` and runs once the folder has been quiet for a few seconds (3 by default, per watch), with every path that changed in the burst — and, if the changes never stop, once the burst has lasted ten times that window, so a folder under constant change still gets a run rather than none. It ignores editor and VCS noise (`.git/`, `.DS_Store`, swap and atomic-save temp files, plus any globs the watch adds) and the writes Iris's own unattended runs make through the file tools, so a watch can write into its own folder without running on its own output; what it cannot see — `run_command`, MCP tools, plugin hooks, the memory tools — is bounded by the breaker instead. Saves that land while the watch's run is going are held and re-fired once when it ends, never dropped and never run alongside it; several watches on one folder share one stream; and a watch whose folder disappears pauses itself with the reason rather than going quiet. The card and `/jobs` say what each burst saw and what it absorbed. Jobs are stored in the conversation database and survive restarts. A background scheduler polls for due jobs every 10 seconds and again right after the Mac wakes; by default a job that came due while asleep fires once and is rescheduled from then, rather than replaying every missed tick. A job can ask for something else with `catch_up`: `skip` drops the missed work entirely and jumps to the next occurrence still in the future, and `replay` fires the most recent missed occurrences one at a time, up to a cap of five (100 at most, and a larger number is lowered to it), with the older ones dropped and counted on the first card ("27 earlier occurrences skipped") — or, when that first fire is refused, held or skipped, on whatever row or card that outcome leaves. `overlap` says what a fire does when the previous run is still going — dropped and recorded by default, or `queue` to hold exactly one and take it when that run ends. A replayed fire is an ordinary one — it asks the gate and counts against the breaker and the budgets — so the burst stops at the first refusal and the job goes back on its cadence; a replayed run that *fails* stops it too, because the failure hands the job to the retry ladder and the rest of the burst would spend that whole backoff in a moment. A job can carry a gate — a HEAD of a URL, a path's mtime and hash, or a script run inside the container with its inputs mounted read-only — checked on the cadence so the job only spends a model turn when something actually changed; a script gate's verdict is the last line of its output, `CHANGED` or `UNCHANGED`, never its exit code, it is reviewed once before the job exists — the script together with the directories it may read and its timeout, since a standing read of your home folder is the part worth refusing — and never runs outside the VM, and three failed checks in a row pause the job rather than letting it fail quietly. A mount is recorded as the directory it resolves to rather than the name it was given, and a source pointed somewhere else after the fact is a failed check rather than a quiet swap. A gate that finds nothing to do leaves a ledger row and no card. A fire runs in a hidden conversation of its own and reports one event card into the pinned "Iris Activity" conversation instead of interrupting whatever you have open, with every run recorded in a ledger; `/jobs` lists what is scheduled, acknowledges a failed run, pauses, resumes or fires a job now, or deletes one, without spending a model turn — and it lists it with the numbers, not just the names: each job's tokens today against its daily budget ("620k / 1M (62%)"), its runs in the last hour against the breaker, where it is on the retry ladder, and whatever its policy does differently from the defaults, with the whole background system's spend for the day under the table. `list_jobs` hands the model the same figures as fields, and the six numbers behind them — per-run and per-job-daily token budgets, the global daily ceiling, the breaker for scheduled jobs, the breaker for watches (higher, because a watch fires once per save-burst), and the run timeout — are steppers in Settings → Advanced → Job Limits, each defaulting to the figure Iris ships with and stepping up or down from the figure its row shows. A run is bounded while it happens — a per-run token budget and a wall-clock timeout end the turn rather than letting it spend unattended, and the Mac is kept awake for the run's own duration and no longer — and a failed run is retried after 1, 5 and 25 minutes before the job pauses itself with "failed 3 times; paused" (a watch fire is not retried — the next save re-runs it with real paths — and neither is a hand-started fire of a watch job, which never had paths at all). A background run is never offered the two job-creating tools and is refused them if it calls one anyway — nothing schedules more work for itself unattended. A job is read-only unless it was created otherwise, and a read-only run's tool surface is an allowlist: it is built with the tools known to read — files, memory and the web — and everything else is refused, including tools added later. A call that reaches the dispatcher anyway is refused there too, recorded with its arguments, and reported as a blocked run on the card, and the turn ends rather than asking again. A run that stopped on a call nobody was there to approve says so in full: the card shows the tool, every argument and a preview of a long body, with Vibecop's verdict on that exact call beside an **Approve and run** button — clicking it dispatches that one call, once, as a tracked run of its own with its own card, and never resumes the job's turn. Three calls are never offered the button and are refused again underneath it: one a read-only job's profile stopped (no approval widens a profile), a write into `~/.iris/config` or `~/.iris/plugins`, where a write grants further permission rather than editing a file, and a granted run's file-tool call outside its grant, which no click can open (re-schedule with a wider grant). The job's profile is re-asked at the moment you click rather than inherited from the blocked run, so a command from a background job still runs in the container or not at all — with the VM gone the click is refused, the approval is left unspent, and nothing falls back to the host. A `mutating` job keeps the whole surface and its commands always run in the `apple/container` VM (the VM routes `run_command`; the other tools run on the host behind the allowlist or the job's grant, as they always do), so such a job can only be created where the runtime is installed and sandboxing is on — a fire that finds the VM gone is refused rather than run on the host, and so is a command from any background run whose VM disappears mid-turn. A `mutating` job can be created with a **grant** — `mounts` (read-write unless `:ro`; the first read-write one is its working directory) and `network` (off by default, which puts the VM on a host-only network: no egress and no LAN, though the Mac's own listeners are still reachable) — under which its commands run with those directories mounted and its `write_file`/`read_file` are allowed unattended inside them, through a descriptor walk that follows no symlink; the grant is re-checked against the disk at every fire, credential stores (`~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.config/gh`, `~/.docker`, `~/.kube`, `~/.azure`, `~/.cargo`, `~/.m2`, `~/.terraform.d`, `~/.oci`, `~/.gem`, `~/.password-store`, `~/Library/Keychains`, `~/Library/Cookies`, `~/Library/Application Support/com.apple.container`, `~/Library/Group Containers`, `~/Library/Containers`) and anything containing them are refused as mounts, a background run cannot change its own workspace, `/jobs` shows the grant, `list_jobs` carries it, and the card says `network` when a run had it. See [docs/jobs.md](docs/jobs.md).
...
*   **Model Status LEDs:** A compact row of retro LED indicators sits between the chat area and input bar, showing real-time status of every inference model: primary tier (color-coded by difficulty), Vibecop Guardian, and all three Prompt Injection Guard tiers. Green = ready, orange = warming/configured, gray = off, with a pulsing glow during active inference. A deeper orange on P2/P3 means that guard tier's model is not downloaded, so the tier is being skipped; **red means the model is installed but failing to load or run**, which is different and worse — that tier is blocking guarded output rather than checking it, and the tooltip carries the error so you can tell a half-unzipped directory from a truncated download. A tier in that state also says so once in the conversation, because a red light you are not looking at is not a warning. Settings → General → "Show model activity LEDs" hides the row if you would rather have the space; the guard notice is not affected by it.
...
4.  `schedule_job`: Creates a recurring job in the conversation database: a five-field `cron` expression with an optional IANA `timezone`, an `intervalSeconds` interval, or the loose `minute`/`hour`/`weekday`/`weekdays` fields (`[2,3,4,5,6]` for every weekday), plus an optional `profile` (`readOnly` by default, or `mutating` for a job that must change something), an optional gate (`gate_url`, `gate_path`, or `gate_script` with `gate_mounts` and `gate_timeout_seconds`) so the job only runs when something changed, and the two policies `overlap` (`skip`/`queue`) and `catch_up` (`coalesce`/`skip`/`replay:N`). Jobs survive restarts. Each fire runs in the background and reports back with a card in Iris Activity.
...
behind a live `AppState`. The lock file is created, not checked and then written, so several
`--run-job` started at the same instant are told apart: one runs and the rest refuse, naming the
pid that has it.
